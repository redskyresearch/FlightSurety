...
contract FlightSuretyApp {
    ...
    */  
    ...
    function fetchFlightStatus(address airline, string memory flight, uint256 timestamp) external {
        ...
        oracleResponseInfoByKey[key] = ResponseInfo({
...
// region ORACLE MANAGEMENT
    ...
    // had to move this out of the struct as it would not compile
    //TypeError: Types in storage containing (nested) mappings cannot be assigned to.
    //--> project:/contracts/ExerciseC6D/ExerciseC6D.sol:156:5:
    // this replaces ResponseInfo.responses
    // Rather than nested struct element, it is now a two dimensional mapping
    ...
    mapping(bytes32 => ResponseInfo) private oracleResponseInfoByKey;
   ...
   // Called by oracle when a response is available to an outstanding request
    ...
    function submitOracleResponse  (uint8 index, address airline, string memory flight, uint256 timestamp, uint8 statusCode) external {
        ...
        bytes32 key = keccak256(abi.encodePacked(index, airline, flight, timestamp)); 
        require(oracleResponseInfoByKey[key].isOpen, "Flight or timestamp do not match oracle request");

        oracleResponsesByKeyAndStatusCode[key][statusCode].push(msg.sender);

        // Information isn't considered verified until at least MIN_RESPONSES
        // oracles respond with the *** same *** information
        emit OracleReport(airline, flight, timestamp, statusCode);
        if (oracleResponsesByKeyAndStatusCode[key][statusCode].length >= MIN_RESPONSES) {
